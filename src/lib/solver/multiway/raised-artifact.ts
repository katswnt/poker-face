...
import type { RaisedRiverIndependentAudit } from "./raised-independent-node";
import type { RaisedRiverRulesAudit } from "./raised-oracle";
import type { RaisedRiverAction } from "./raised-river-game";
import { raisedRiverDecisionFacts, type RaisedDecisionFacts } from "./raised-teaching";

export type SerializedRaisedRiverStrategy = Readonly<Record<
  string,
  Readonly<Partial<Record<RaisedRiverAction, number>>>
>>;

export interface RaisedRiverConvergenceCheckpoint {
...
export interface RaisedRiverSolveArtifactPayload {
  readonly schemaVersion: 1;
  readonly game: "three-player-raised-river-v1";
  readonly resultLabel: "bounded-multiway-raised-river-proof";
  readonly rules: {
    readonly version: 1;
    ...
    readonly openingBet: number;
    readonly allInRaiseTo: number;
  ...
  readonly tree: {
    ...
    readonly terminalActionHistories: 43;
  };
  readonly strategy: SerializedRaisedRiverStrategy;
  readonly value: readonly number[];
  ...
  readonly convergence: readonly RaisedRiverConvergenceCheckpoint[];
  readonly rulesAudit: RaisedRiverRulesAudit;
  readonly independentChecks: RaisedRiverIndependentAudit;
  ...
  readonly decisions: readonly RaisedDecisionFacts[];
}

export interface RaisedRiverSolveArtifact extends RaisedRiverSolveArtifactPayload {
  readonly payloadHash: string;
}

export const RAISED_RIVER_ACCEPTANCE = {
...
export function serializeRaisedRiverStrategy(
  strategy: MultiwayBehavioralStrategy<RaisedRiverAction>,
): SerializedRaisedRiverStrategy {
  const serialized: Record<string, Partial<Record<RaisedRiverAction, number>>> = {};
...
export function deserializeRaisedRiverStrategy(
  serialized: SerializedRaisedRiverStrategy,
): MultiwayBehavioralStrategy<RaisedRiverAction> {
  ...
  ) throw new Error("Serialized raised strategy does not match the game information sets");
  const strategy = new Map(index.informationSets.map(definition => {
    const stored = serialized[definition.key];
    if (!stored) throw new Error(`Missing serialized raised strategy at ${definition.key}`);
    if (
      Object.keys(stored).length !== definition.actions.length ||
      Object.keys(stored).some(action => !definition.actions.includes(action as RaisedRiverAction))
    ) throw new Error(`Serialized raised actions do not match the game at ${definition.key}`);
...
export function createRaisedRiverArtifactPayload(
  result: MultiwayCfrSolveResult<RaisedRiverAction>,
  rulesFingerprint: string,
  rulesAudit: RaisedRiverRulesAudit,
  independentChecks: RaisedRiverIndependentAudit,
): RaisedRiverSolveArtifactPayload {
  if (result.gameId !== raisedRiverV1Game.id) throw new Error(`Cannot create raised artifact from ${result.gameId}`);
  if (!/^[a-f0-9]{64}$/.test(rulesFingerprint)) throw new Error(`Invalid rules fingerprint ${rulesFingerprint}`);
  const grade = gradeMultiwayStrategy(raisedRiverV1Game, result.averageStrategy, result.index);
  ...
  const independentPassed =
    independentChecks.reducedBestResponse.maximumValueDifference <= RAISED_RIVER_ACCEPTANCE.maximumAuditError &&
    independentChecks.hiddenInformationBoundary.cheatingAdvantage > 0.01 &&
    independentChecks.priorArtifacts.stageOneHashValid &&
    independentChecks.priorArtifacts.headsUpHashValid;
  const scenario = raisedRiverV1Game.scenario;
  return {
    schemaVersion: 1,
    game: "three-player-raised-river-v1",
    resultLabel: "bounded-multiway-raised-river-proof",
    rules: {
      ...
      openingBet: scenario.betSize,
      allInRaiseTo: scenario.raiseTo,
    ...
    tree: {
      ...
      compatiblePrivateDeals: raisedRiverV1Game.deals.length,
      terminalActionHistories: 43,
    },
    strategy: serializeRaisedRiverStrategy(result.averageStrategy),
    value: grade.value,
    ...
    independentOpenSourceReference: {
      ...
      note: "The audited open-source river solvers are heads-up; none matched this exact three-player raised game.",
    },
    acceptance: {
      ...
      passed:
        grade.maximumUnilateralGain <= RAISED_RIVER_ACCEPTANCE.maximumUnilateralGain &&
        maximumRulesError <= RAISED_RIVER_ACCEPTANCE.maximumAuditError &&
        maximumProfileZeroSumError <= RAISED_RIVER_ACCEPTANCE.maximumAuditError &&
        independentPassed,
    },
    decisions: raisedRiverDecisionFacts(raisedRiverV1Game, result.averageStrategy),
  };
}

export function stringifyRaisedRiverArtifact(artifact: RaisedRiverSolveArtifact): string {
