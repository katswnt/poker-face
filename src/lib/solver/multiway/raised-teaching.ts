...
export interface RaisedOpponentMarginal {
...
export interface RaisedJointOpponentWeight {
  readonly key: string;
  readonly hands: readonly { readonly player: number; readonly cards: RiverCombo }[];
  readonly probability: number | null;
}

export interface RaisedResponseFrequency {
  readonly player: number;
  readonly action: RaisedRiverAction;
  readonly probability: number | null;
}

export interface RaisedOutcomeBreakdown {
...
export interface RaisedActionFact {
  readonly action: RaisedRiverAction;
  ...
  readonly immediateResponses: readonly RaisedResponseFrequency[];
  readonly immediateOpponentFoldProbability: number | null;
  readonly outcomes: RaisedOutcomeBreakdown;
...
export interface RaisedDecisionFacts {
  ...
  readonly jointOpponentRange: readonly RaisedJointOpponentWeight[];
  readonly opponentMarginals: readonly RaisedOpponentMarginal[];
  readonly actions: readonly RaisedActionFact[];
}

interface ReachedState {
  readonly state: RaisedRiverState;
...
function strategyEntry(
  ...
  if (!entry) throw new Error(`Missing raised river strategy at ${key}`);
  return entry;
}

function stateKey(state: RaisedRiverState): string {
...
function terminalSummary(
  ...
  state: RaisedRiverState,
  player: number,
): ContinuationSummary {
  const node = game.node(state);
  if (node.kind !== "terminal") throw new Error("Expected a terminal raised river state");
...
function continuationEvaluator(
  ...
  const evaluate = (state: RaisedRiverState, player: number): ContinuationSummary => {
    ...
    if (Math.abs(outcomeMass(result) - 1) > PROBABILITY_TOLERANCE) {
      throw new Error(`Raised continuation outcomes sum to ${outcomeMass(result)}, not 1`);
    }
    cache.set(key, result);
    return result;
  };
...
function nullOutcomes(): RaisedOutcomeBreakdown {
  return {
    playerFolds: null,
...
function assertSamePublicState(game: RaisedRiverGame, states: readonly ReachedState[]): void {
  ...
  for (const reached of states.slice(1)) {
    ...
    if (actual !== expected) throw new Error("A raised information set has inconsistent public state");
  }
}
...
function immediateResponses(
...
): readonly RaisedResponseFrequency[] {
  let responsePlayer: number | null = null;
  let responseActions: readonly RaisedRiverAction[] | null = null;
  ...
  for (const reached of states) {
    ...
    if (responsePlayer === null) {
      ...
      throw new Error(`Raised action ${action} has inconsistent immediate response states`);
    }
...
/**
 * Exact, blocker-aware teaching facts. Hidden deals are grouped before any action
 * value or opponent range is reported.
 */
export function raisedRiverDecisionFacts(
  game: RaisedRiverGame,
  strategy: MultiwayBehavioralStrategy<RaisedRiverAction>,
): readonly RaisedDecisionFacts[] {
  ...
  const visit = (state: RaisedRiverState, chanceReach: number, reach: readonly number[]): void => {
    const node = game.node(state);
  ...
  return index.informationSets.map(definition => {
    ...
    const jointHands = new Map<string, RaisedJointOpponentWeight["hands"]>();
    for (const reached of states) {
    ...
    const summaries = definition.actions.map(action => {
      ...
      if (Math.abs(outcomeMass(summary) - 1) > PROBABILITY_TOLERANCE) {
        throw new Error(`Raised action outcomes at ${definition.key} do not sum to 1`);
      }
      return summary;
    });
